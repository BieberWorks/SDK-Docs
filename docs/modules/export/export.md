...
| Format | Constant | MIME Type | Library |
|---|---|---|---|
| PDF | `ExportFormatId.Pdf` | `application/pdf` | PDFsharp 6.x (MIT) |
| CSV | `ExportFormatId.Csv` | `text/csv` | CsvHelper 33.x |
| Excel | `ExportFormatId.Excel` | `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` | ClosedXML 0.105.x |
| Word | `ExportFormatId.Word` | `application/vnd.openxmlformats-officedocument.wordprocessingml.document` | Register a custom formatter |
| OpenDocument | `ExportFormatId.OpenDocument` | `application/vnd.oasis.opendocument.text` | Register a custom formatter |
...
await using var result = await exportService.ExportAsync(
    stream,
    new ExportRequest(ExportFormatId.Csv, FileName: "orders_june"));
...
|---|---|---|---|
| `Format` | `ExportFormatId` | — | Target format (required); a `readonly record struct` wrapping a `string Value`, with well-known constants `ExportFormatId.Csv/Excel/Pdf/Word/OpenDocument` |
...
<ExportButton TItem="OrderRow"
              Data="@orders"
              Formats="@([ExportFormatId.Csv, ExportFormatId.Excel])"
              FileName="orders"
...
| `Formats` | `IReadOnlyList<ExportFormatId>` | CSV, Excel, PDF | Formats shown in the menu. Single entry renders a plain button. |
